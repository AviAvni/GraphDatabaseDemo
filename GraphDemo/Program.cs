...
namespace GraphDemo
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Save("stops.txt",
                 "CREATE (s:Stop {id: {stop_id}, name: {stop_name}, lat: {stop_lat}, lon: {stop_lon}, parent_station: {parent_station}, location_type: {location_type}, code: {stop_code}, desc: {stop_desc}, zone: {zone_id}})",
                 1000,
                 null,
            ...
            Save("stop_times.txt",
                 "MATCH (t:Trip {id: {trip_id}}), (s:Stop {id: {stop_id}}) CREATE (t)<-[:PART_OF_TRIP]-(st:Stoptime {arrival_time: {arrival_time}, departure_time: {departure_time}, stop_sequence: toInteger({stop_sequence}), pickup_type: {pickup_type}, drop_off_type: {drop_off_type}, shape_dist_traveled: {shape_dist_traveled}})-[:LOCATED_AT]->(s)",
                 1000,
                 null,
